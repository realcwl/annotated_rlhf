...
class ActorCritic(nn.Module):
    def __init__(
        ...
        self.value_head = nn.Sequential(
            ...
            # Any dimensions with a size of 1 at the end are removed.
            Rearrange("... 1 -> ..."),
        )
    ...
    def generate(
        ...
        # actions: (1, N_actions)
        # This generation doesn't calculate gradients as it's a pure sample operation.
        ...
        # prompt mask: [1, 1, 1, ..., 0, 0, 0] of length N_seq
        ...
        # action mask: [0, 0, 0, ..., 1, 1, 1] of length N_seq
        ...
        # I think this is to calculate KL divergence later?
        # sequence: (1, seq_len)
    ...
    def forward(self, x, mask=None, return_values=True):
        # x: (B, T), where T is N_seq, which is the entire sequence including prompt.
        # action logits: (B, T, C), where T is N_seq
        # Note that action logits is a bit confusing as the logits are of the entire sequence.
        action_logits = self.actor_palm(x, finetune_scope=self.actor_lora_scope)

        if not return_values:
            return action_logits, None

        # critic_embeds: (B, T, d) - which act as the value function.
        ...
        if self.pooled_values:
            ...
            # critic_embeds will be of shape (B, d) as the values are pooled along the sequence dimension.
            # critic_embeds only has actions, not the prompt.
            critic_embeds = masked_mean(critic_embeds, mask, dim=1)

        # critic_embeds: (B, 1, d) -> (B, 1, 1) -> (B,)
        # the value is not bounded!
...
def clipped_value_loss(values, rewards, old_values, clip):
    """
    values, reward, old_values are all of shape (B, 1)
    """
...
class RLHFTrainer(nn.Module):
    ...
    def generate(
        ...
        **kwargs,
        ...
        (actions, sequences, mask, prompt_mask, action_logits, _) = (
    ...
    def learn(self, memories: Deque[Memory]):
        ...
        for _ in range(self.epochs):
            for (
                ...
                # * sequence: (B, T) of token ids.
                # * prompt_mask: (B, T) of 1s and 0s. denoting which part is the prompt.
                # * mask: (B, T) of 1s and 0s. denoting which part is non padding (including EOS).
                # * action_prob: (B, T, C) of probabilities of the entire sequence. With the first token being equal probability.
                # * action_log_prob: (B, N_actions) of log probabilities of the actions.
                # * reward: (B,) whose value is a single binned reward value e.g. [0-4]
                # * value: (B,) a unbounded value estimation single value of the sequence.
                action_masks = ~prompt_masks & masks

                # This time we are updating the actor critic model! Actor is our policy and critic!
                action_logits, values = self.actor_critic(sequences, mask=action_masks)

                # Align as the action is generated by the previous token's logits.
                action_logits = shift(
                ...
                # How do approximate the KL divergence between the old policy and the new policy?
                # Well, KL divergence is basically how different 2 distributions assign probability to the same sequence.
                # And that can be approximated by the log probability of the old policy and the new policy, for the same action.
                if self.kl_div_loss_weight > 0:
                    # Generate the KL divergence penalty between the old policy and new model. Using its
                    # probability assignment to the mask.
                    kl_penalty = (
                ...
                # This is a mathematical trick to calculate the loss, because
                # a/b = exp(log(a) - log(b))
                ...
                # Additionally add entropy to the loss, to encourage exploration.
                policy_loss = -torch.min(surr1, surr2) - self.beta_s * entropies

                # combine losses along batch.
                ...
                # Because reward function isn't differentiable, we are not going to update the reward model.
                # Instead, we promote actions that leads to desirable rewards and penalize those that don't.
                self.actor_optim.step()
                self.actor_optim.zero_grad()

                # calculate value loss and update value network separate from policy network
                # This is the mean squared error loss between the value estimation and the actual reward.
                # Note that reward, old_values are detached (first one is explicit, second one is using generate function which is no_grad()).
                # Thus, we are only updating the value network (which is a deep copy of the PALM network).
    ...
    def train(
        ...
        for eps in tqdm(range(num_episodes), desc="episodes"):
            for timestep in range(max_timesteps):
                ...
                # select only 1 state
                # size: (T), [tok_1, tok_2, ..., tok_T, pad, pad, pad]
                state = self.prompt_token_ids[rand_prompt_index]

                # remove padding from state

                # state_mask: [1, 1, ..., 0, 0, 0] where 0 is pad location.
                state_mask = state != self.pad_value

                # select state that is not padded.
                # [tok_1, tok_2, ..., tok_T] // removed: [pad, pad, ..., pad]
                state = state[state_mask]

                # get predicted sequence
                # Note that the reward generation is NOT trainable as generate is marked no_grad.
                (actions, sequence, mask, prompt_mask, action_logits, value) = (
                    self.actor_critic.generate(
                        # add a batch size, which is 1.
                ...
                # During RLHF single memory sampling, this action_logits is
                # [tok_1, tok_2, ..., EOS] without padding, because it's the only sample.

                # Need to shift along sequence dimension by 1, because actions are predicted by the last
                # token's logits.
                action_logits = shift(action_logits, shift=1, dim=-2)

                # get action probabilities
                # action_probs: (1, T, C) with last dimension being the probability, instead of raw logits.
                # This includes the entire sequence.
                action_prob = action_logits.softmax(dim=-1)

                action_len = actions.shape[-1]

                # Firstly, it use the sequence (of indexes) to gather the probabilities along the last dimension.
                # Which is exactly the conditional probability P(y_t | y_1, y_2, ..., y_{t-1}, x) for the generated
                # sequence.
                # Secondly, it calculates the log probability of the actions taken. And remove the last dimension.
                action_log_prob = log_prob(action_prob, sequence)

                # This is where the action is separated from the prompt.
                # action_log_prob shape: (1, N_actions), where each cell is the log probability.
                action_log_prob = action_log_prob[:, -action_len:]

                # (1, N_actions) -> (N_actions, )
                ...
                # sequence: (1, T) contains [tok_1, ..., tok_N_prompt, tok_a1, tok_a2, ..., tok_N_action, EOS]
                # prompt_mask: (1, T) contains [1, 1, ..., 0, 0, 0] and has N_prompt 1s.
                sequence = rearrange(sequence, "n -> 1 n")
                prompt_mask = rearrange(prompt_mask, "n -> 1 n")

                # mask is to hide the EOS token.
                mask = default(
                    mask,
                    lambda: torch.ones(sequence.shape, dtype=torch.bool, device=device),
                )

                # reward is on top of the entire sequence, including prompt. We provide the prompt mask
                # so that it can understand which part is prompt and which part is response.
                ...
                # The memory has
                # * sequence: (1, T) of token ids.
                # * prompt_mask: (1, T) of 1s and 0s. denoting which part is the prompt.
                # * mask: (1, T) of 1s and 0s. denoting which part is the actions (including EOS).
                # * action_prob: (1, T, C) of probabilities of the entire sequence. With the first token being equal probability.
                # * action_log_prob: (1, N_actions) of log probabilities of the actions.
                # * reward: (1, RewardBin) a single binned reward value e.g. [0-4]
                # * value: (1,) a unbounded value estimation single value of the sequence.
                memories.append(
                    Memory(
                        *map(
                            detach_to_cpu_,
