...
class PaLM(nn.Module):
    ...
    def generate(
        ...
        **kwargs,
        ...
        # During RLHF, prompt is of shape (1, T)
        ...
        for _ in wrapper_fn(range(sample_num_times)):
            ...
            # During RLHF training, select only last logits and embedding as it's the next token.
            # logits: (1, 1, C)
            # embeds: (1, 1, d)
            ...
            if exists(eos_token):
                ...
                # If all sequences generated EOS token, break the loop.
                if is_eos_tokens.any(dim=-1).all():
                    # mask out everything after the eos tokens
                    # step 1: shift right so that we keep the EOS token.
                    shifted_is_eos_tokens = F.pad(is_eos_tokens, (1, -1))

                    # step 2: select padding value
                    # [
                    #   [0, 0, 0, ..., 1, 1, 1]
                    # ]
                    # The cumsum will make sure that the first EOS token's occurrance will
                    # make all subsequent tokens to be masked out (with cumsum value >= 1).
                    mask = shifted_is_eos_tokens.float().cumsum(dim=-1) >= 1

                    # step 3: mask out the input, keep the EOS token (because we shifted once)
                    # output becomes:
                    # [
                    #   [tok_1, tok_2, ...,   EOS, pad, pad, pad]
                    #   [tok_1, tok_2, tok_3, ..., EOS, pad, pad]
                    # ]
        ...
        # only return action sequence. This is the default choice
        return out[..., n:]
    ...
    def forward(
        ...
        """
        x: (B, T), the second dimension is a integer number representing the token id.
        """
        if return_loss:
            x, labels = x[:, :-1], x[:, 1:]
        ...
        # x: (B, T)
        # x: (B, T, C) after embedding.
        x = self.token_emb(x)

        # This is used to denote the prompt and response sections. Which is critical
        # for the reward model to understand which section is prompt and which section is response.
